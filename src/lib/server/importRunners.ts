// Extracted from src/routes/api/import/+server.ts so the queue system
// (importQueue.ts) can drive the same catalogue/lectures import logic that
// used to be started directly from the POST handler. Behaviourally
// unchanged aside from added isPaused() support (pause without deleting
// data, catalogue restarts fresh on resume since it is idempotent;
...
export async function runCatalogueImport(periodeId: string, lang: string, log: (msg: string) => void, isCancelled: () => boolean, isPaused: () => boolean) {
    ...
    log(`Datenbank: data/${periodeId}/${lang}/lectures.db`);
    ...
    try {
        ...
        if (isCancelled() || isPaused()) {
            db.exec(`ROLLBACK`);
            if (isCancelled()) log(`Abgebrochen nach ${nodeCount} Knoten — nicht committet.`);
            else log(`Pausiert nach ${nodeCount} Knoten — nicht committet. Wird beim Fortsetzen komplett neu geholt (Katalog-Import ist idempotent).`);
            return;
        }
        db.exec(`COMMIT`);
        ...
        if (firstParsedSample) {
            const sample: { hk: number; unibasId: number } = firstParsedSample;
            const rawRow = db.prepare(`SELECT hierarchy_key, unibas_id, title, node_type FROM lecture_catalog WHERE hierarchy_key = ?`).get(sample.hk) as any;
            log(`  [DEBUG] Stichprobe key=${sample.hk}: gespeichert=${JSON.stringify(rawRow)} (erwartet unibas_id=${sample.unibasId})`);
        }
    } catch (err: any) {
...
export interface LecturesImportProgress {
    processedIds: number[];
    success: number;
    failed: number;
}

export async function runLecturesImport(
    periodeId: string,
    lang: string,
    log: (msg: string) => void,
    isCancelled: () => boolean,
    isPaused: () => boolean,
    resumeFrom?: LecturesImportProgress,
    onProgress?: (progress: LecturesImportProgress) => void
) {
    const db = getDb(periodeId, lang);

    // Only reset the "imported" stamp on a genuinely fresh run — a resumed
    // run continuing after a pause should keep whatever was already there
    // (it'll be overwritten with the final numbers once the whole batch
    // actually completes).
    if (!resumeFrom) {
    ...
    log(`${rows.length} eindeutige Vorlesungen in data/${periodeId}/${lang}/lectures.db`);

    const alreadyProcessed = new Set(resumeFrom?.processedIds ?? []);
    const processedIds: number[] = [...(resumeFrom?.processedIds ?? [])];
    let success = resumeFrom?.success ?? 0;
    let failed = resumeFrom?.failed ?? 0;
    if (resumeFrom) {
        log(`Fortsetzen: ${processedIds.length}/${rows.length} bereits verarbeitet (${success} erfolgreich, ${failed} fehlgeschlagen).`);
    }

    for (const row of rows) {
        if (alreadyProcessed.has(row.unibas_id)) continue;
        if (isCancelled()) {
            log(`Abgebrochen nach ${success + failed}/${rows.length} Vorlesungen.`);
            break;
        }
        if (isPaused()) {
            log(`Pausiert nach ${success + failed}/${rows.length} Vorlesungen.`);
            break;
        }
        try {
        ...
        processedIds.push(row.unibas_id);
        onProgress?.({ processedIds, success, failed });
    }
