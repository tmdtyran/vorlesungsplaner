    ...
    <div class="border-t border-slate-200 bg-slate-50 p-5 overflow-y-auto max-h-72">
        <div class="flex items-start justify-between gap-4">
            ...
            <div class="flex items-center gap-2 shrink-0">
                ...
                >
                    {t('Weiteres')} <span aria-hidden="true">→</span>
                </button>
                <button
                    onclick={onClose}
        ...
        <div class="mt-3 grid grid-cols-2 gap-x-8 gap-y-2 text-sm lg:grid-cols-4">
            {#if detail.semester}
                <div><span class="text-xs text-slate-500 block">{t('Semester')}</span>{detail.semester}</div>
            {/if}
            {#if detail.language}
                <div><span class="text-xs text-slate-500 block">{t('Sprache')}</span>{detail.language}</div>
            {/if}
            {#if detail.faculty}
                <div><span class="text-xs text-slate-500 block">{t('Fakultät')}</span>{detail.faculty}</div>
            {/if}
            {#if detail.offered_by}
                <div><span class="text-xs text-slate-500 block">{t('Angeboten von')}</span>{detail.offered_by}</div>
            {/if}
            {#if detail.lecturers}
                <div class="col-span-2"><span class="text-xs text-slate-500 block">{t('Dozierende')}</span>{detail.lecturers}</div>
            {/if}
            {#if detail.assessment_format}
                <div class="col-span-2"><span class="text-xs text-slate-500 block">{t('Prüfungsform')}</span>{detail.assessment_format}</div>
            {/if}
        </div>
        {#if detail.recurringTimes?.length > 0}
            <div class="mt-3">
                <p class="text-xs font-medium text-slate-500 uppercase tracking-wide mb-1.5">{t('Regelmässigkeit')}</p>
        ...
        {#if detail.content}
            <div class="mt-3">
                <p class="text-xs font-medium text-slate-500 uppercase tracking-wide mb-1.5">{t('Beschreibung')}</p>
                <p class="text-sm text-slate-700 leading-relaxed whitespace-pre-line">{detail.content}</p>
            </div>
        {:else if detail.imported_at}
            <div class="mt-3">
                <p class="text-xs text-slate-400">{t('Keine Beschreibung vorhanden.')}</p>
            </div>
        {:else}
            <div class="mt-3">
                <p class="text-xs text-slate-400">{t('Keine Beschreibung verfügbar — führe "Import All Lectures" aus, um Details zu laden.')}</p>
            </div>
        {/if}
    </div>
{/if}
